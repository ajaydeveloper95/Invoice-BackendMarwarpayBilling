import welexopayInvoiceDB from "../models/welexopay.model.js"
...
export const generateZIP = asyncHandler(async (req, res) => {
    try {
        ...
        res.setHeader('Content-disposition', 'attachment; filename=InvoiceWelexopay.zip');
        res.download(downlaodpath, "InvoiceWelexopay.zip")
    } catch (error) {
...
export const storeData = asyncHandler(async (req, res) => {
    try {
        ...
        // store Data
        await welexopayInvoiceDB.deleteMany({});
        let data1 = await welexopayInvoiceDB.insertMany(csvData);
...
export const generatePdf = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const db = await welexopayInvoiceDB.find();
        for (let i = 0; i < db.length; i++) {
            ...
            const page = await browser.newPage();
            let content = await compileTemplete("welexopay", data)
            // let content = await compileTemplete("mediterrance", data)
            await page.setContent(content);
            await page.setViewport({ width: 1080, height: 1024 });
            // genrate pdf
