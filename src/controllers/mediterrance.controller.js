import mediInvoiceDB from "../models/mediInvoice.model.js"
...
export const generateZIP = asyncHandler(async (req, res) => {
    try {
        ...
        res.setHeader('Content-disposition', 'attachment; filename=InvoiceMediZip.zip');
        res.download(downlaodpath, "InvoiceMediZip.zip")
    } catch (error) {
...
export const storeData = asyncHandler(async (req, res) => {
    try {
        ...
        // store Data
        await mediInvoiceDB.deleteMany({});
        let data1 = await mediInvoiceDB.insertMany(csvData);
...
export const generatePdf = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const db = await mediInvoiceDB.find();
