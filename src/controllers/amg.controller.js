import amgInvoiceDB from "../models/amgInvoice.model.js"
...
export const generateZIP = asyncHandler(async (req, res) => {
    try {
        ...
        res.setHeader('Content-disposition', 'attachment; filename=InvoiceAmgZip.zip');
        res.download(downlaodpath, "InvoiceAmgZip.zip")
    } catch (error) {
...
export const test = asyncHandler(async (req, res) => {
    try {
        ...
        let content = await compileTemplete("index1", data)
...
export const storeData = asyncHandler(async (req, res) => {
    try {
        ...
        // store Data
        await amgInvoiceDB.deleteMany({});
        let data1 = await amgInvoiceDB.insertMany(csvData);
...
export const generatePdf = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const db = await amgInvoiceDB.find();
