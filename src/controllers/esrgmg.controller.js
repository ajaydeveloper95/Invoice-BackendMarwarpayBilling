import esrgmgInfoInvoiceDB from "../models/esrgmginfo.model.js"
...
export const generateZIP = asyncHandler(async (req, res) => {
    try {
        ...
        res.setHeader('Content-disposition', 'attachment; filename=InvoiceEsrgmgInfo.zip');
        res.download(downlaodpath, "InvoiceEsrgmgInfo.zip")
    } catch (error) {
...
export const test = asyncHandler(async (req, res) => {
    try {
        ...
        let content = await compileTemplete("index2", data)
...
export const storeData = asyncHandler(async (req, res) => {
    try {
        ...
        // store Data
        await esrgmgInfoInvoiceDB.deleteMany({});
        let data1 = await esrgmgInfoInvoiceDB.insertMany(csvData);
...
export const generatePdf = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const db = await esrgmgInfoInvoiceDB.find();
        for (let i = 0; i < db.length; i++) {
            ...
            const page = await browser.newPage();
            let content = await compileTemplete("esrgmginfotech", data)
            // let content = await compileTemplete("mediterrance", data)
            await page.setContent(content);
            await page.setViewport({ width: 1080, height: 1024 });
            // genrate pdf
